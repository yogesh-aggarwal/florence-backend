...
async function applyCouponIfApplicable(
	...
	await CouponModel.updateOne(
		{ code: couponCode },
		{ $inc: { usersUsed: 1 }, $push: { usedBy: userID } }
	)
	...
	if (coupon.percentage) {
		...
		if (coupon.maxAmount && discountedAmount > coupon.maxAmount)
			discountedAmount = coupon.maxAmount
...
export async function calculateOrderDiscountQuotation(
	...
	if (appliedCoupon) {
		const couponDiscount = await applyCouponIfApplicable(
			amount,
			appliedCoupon,
			userID
		)
