...
export default async function getPlatformHomeData(req: Request, res: Response) {
   try {
      ...
      if (!data) {
         return res.status(500).send({ error: ResponseMessages.INTERNAL_SERVER_ERROR })
      }
      ...
      if (!user) {
         return res.status(404).send({ error: ResponseMessages.AUTH_USER_DOES_NOT_EXIST })
      }
      ...
      return res.status(200).send({ message: ResponseMessages.SUCCESS, data: response })
   } catch (error) {
